    ...
    <div class="club-activity">
        ...
        >
            ...
            <el-table-column label="详情" align="center">
                <template slot-scope="{ row }">
                    <el-button type="success" plain @click="showDetailPage(row.detail)">查看详情页</el-button>
                </template>
            </el-table-column>
        ...
        <el-dialog :title="handleType" :visible.sync="dialogFormVisible" @close="closeDialog()">
            ...
            >
                ...
                <el-form-item label="详情">
                    <textarea style="width:100%" v-model="temp.detail"></textarea>
                </el-form-item>
                <el-form-item label="学校">
                    <textarea style="width:100%" v-model="temp.schoolId"></textarea>
                </el-form-item>
            </el-form>
...
<script>
    ...
    import { ableClubActivity, disAbleClubActivity } from '../../../../api/clubActivity';
    ...
    export default {
        ...
        methods: {
            ...
            async ableAndDisable(row) {
                ...
                if (state) {
                    const res = await ableViewpager(row.id);
                    if (res.code === 10000) {
                        this.$message({
                            type: 'info',
                            message: '操作成功'
                    ...
                    const res2 = await disAbleViewpager(row.id);
                    if (res2.code === 10000) {
                        this.$message({
                            type: 'info',
                            message: '操作成功'
            ...
            async handleDelete(row) {
                const res = await deleteviewpager(row.id);
                if (res.code === 10000) {
                    this.$message({
                        type: 'info',
                        message: '操作成功'
            ...
            async update() {
                ...
                const res = await updateviewpager(params);
                if (res.code === 10000) {
                    this.$message({
                        type: 'info',
                        message: '操作成功'
            ...
            //插入
            async insert() {
                const res = await addviewpager(this.temp);
                if (res.code === 10000) {
                    this.$message({
                        type: 'info',
                        message: '操作成功'
